...
def extract_ablation_parameters(run_id: str, experiment_name: str, run_info: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if prompt:
        ...
        if experiment_name in ["exp0_baseline", "exp0_baseline_indep"]:
...
def process_experiment(exp_dir: Path, results_dir: Path) -> None:
    ...
    exp_output_dir = results_dir / exp_name
    exp_output_dir.mkdir(exist_ok=True)
    
    # Create more descriptive filename
    descriptive_name = get_descriptive_experiment_name(exp_name)
    consolidated_filename = f"{descriptive_name}_raw_data.csv"
    
    # Save consolidated runs data in experiment-specific directory
    exp_consolidated_path = exp_output_dir / consolidated_filename
    
    # Add methodology header for raw data
    raw_data_header = f"""# RAW EXPERIMENTAL DATA - METHODOLOGY
# This file contains raw experimental data consolidated from individual runs
# 
# Data Details:
# - Experiment: {exp_name}
# - Total runs: {len(runs_df)}
# - Data source: Individual experiment runs with metrics calculated per run
# - Metrics included: Fidelity, diversity, cross-consistency
# - Statistics: Mean and standard deviation calculated per run
# 
# Each row represents one experimental run with its calculated metrics
# Raw data before any averaging or statistical aggregation
#
# Generated by: consolidate_experiments.py
# Date: {pd.Timestamp.now().strftime('%Y-%m-%d %H:%M:%S')}
#
"""
    
    with open(exp_consolidated_path, 'w') as f:
        f.write(raw_data_header)
        runs_df.to_csv(f, index=False)
    
    print(f"  Saved consolidated data to {exp_consolidated_path}")
    
    # Also save in main results directory for compatibility
    main_consolidated_path = results_dir / f"{exp_name}_consolidated.csv"
    runs_df.to_csv(main_consolidated_path, index=False)
